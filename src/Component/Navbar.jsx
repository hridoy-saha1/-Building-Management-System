...
import { FaUserCircle, FaBars, FaTimes, FaBullhorn } from 'react-icons/fa';
...
const Navbar = () => {
  ...
  const loggedInRoutes = [
    { path: "/", label: "Home" },
    { path: "/apartments", label: "Apartments" },
    { path: "/dashboard/announcements", label: "Announcement" },
              ...
              <>
                {loggedInRoutes.map((route) => (
                  <Link
                    key={route.path}
                    to={route.path}
                    className="text-black hover:text-yellow-500 transition flex items-center gap-1"
                  >
                    {route.label === "Announcement" && <FaBullhorn />}
                    {route.label}
                  </Link>
                ))}
                {/* User Dropdown */}
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 hidden group-hover:block bg-white text-black border rounded-md shadow-lg w-44 z-50">
                    ...
                    <hr />
                    <Link to="/dashboard/profile" className="block px-4 py-2 hover:bg-gray-100 text-sm">Profile</Link>
                    <button
              ...
              <>
                {loggedInRoutes.map((route) => (
                  <Link
                    key={route.path}
                    to={route.path}
                    className="block hover:text-yellow-500 transition flex items-center gap-1"
                    onClick={() => setMenuOpen(false)}
                  >
                    {route.label === "Announcement" && <FaBullhorn />}
                    {route.label}
